...
1. **credentials** — read the OAuth `accessToken`, **strictly read-only** (Pulse must NEVER refresh Claude Code's OAuth token or write to its credential stores — refresh tokens are single-use, so an uncoordinated refresh revokes the user's Claude Code login). Secret-read order: `~/.claude/.credentials.json` first (JSON path `claudeAiOauth.accessToken`, fallback `accessToken`); on macOS only, if the file is absent, fall back to keychain item `Claude Code-credentials` (`security find-generic-password -s ... -w` / `SecItemCopyMatching`).
   **CredentialCache contract (same state machine in all three ports — do not simplify):** the secret is cached in memory and every poll does only a *prompt-free fingerprint read* — file mtime (`"file:<mtime>"`), else the keychain item's `mdat` attribute (`"keychain:<mdat>"`; `security find-generic-password -s ...` **without `-w`** / `SecItemCopyMatching` with `kSecReturnAttributes` only — attribute reads never trigger keychain ACL prompts), else null = no credentials. The secret is re-read only when the fingerprint changes. On HTTP 401/403 the cache is invalidated and the request retried **once** iff the store already holds a different token; while the fingerprint still equals the rejected/denied one, polls throw without touching the secret (this gate is what prevents a keychain-prompt storm — a bare "re-read on 401" reintroduces it). Manual "Refresh Now" (`force`) bypasses only the rejection gate, never a healthy cache. Env var `CLAUDE_USAGE_TOKEN` bypasses everything (useful for testing / a `claude setup-token` token).
